...
class ZSet[T]:
    """
        ...
        A Z-set with elements all having a weight of 1 is a set. One where all 
        weights are strictly positive is a bag, and one where they are either +1
        or -1 is a diff
    """
    ...
    def __eq__(self, other: object) -> bool:
        """Two Z-sets are equal if they have same elements with same weights"""
        if not isinstance(other, ZSet):
            return False
        return self.inner == other.inner
    
    def __contains__(self, item: T) -> bool:
        """An item is in this Z-set if its has non-zero weight"""
        return self.inner.__contains__(item)

    def __getitem__(self, item: T) -> int:
        """Returns the weight of the item (0 if not present)"""
        if item not in self:
            return 0
...
class ZSetAddition[T](AbelianGroupOperation[ZSet[T]]):
    """Defines ZSet addition operation for Z-sets, forming an Abelian group"""
