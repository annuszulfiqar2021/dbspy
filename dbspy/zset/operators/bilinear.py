...
from dbspy.stream.operators.linear import Delay, Integrate, LiftedDelay, LiftedIntegrate
from dbspy.stream import LiftedGroupAdd, LiftedGroupNegate
from dbspy.zset import ZSet, ZSetAddition
from dbspy.zset.functions.bilinear import JoinCmp, PostJoinProjection, join
...
class DeltaLiftedDeltaLiftedJoin(BinaryOperator[Stream[ZSet[T]], Stream[ZSet[R]], Stream[ZSet[S]]]):
    """
    Incrementally computes the Z-Set join between two streams element-wise. Equivalent to - but keeps less state - incrementalizing a doubly-lifted join. See :func:`~dbspy.stream.operators.Incrementalize2` to grasp what it means to incrementalize a singly-lifted join.
    """
